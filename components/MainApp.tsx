...
import { OrderInspectionPage } from './OrderInspectionPage';

type View = 'Dashboard' | 'Order Service' | 'Inspection' | 'Diagnostics' | 'Assistant' | 'Finalize' | 'Report';
...
export const MainApp: React.FC<MainAppProps> = ({ user, onLogout }) => {
  ...
  const handleTabChange = (tab: View | string) => {
    ...
    if (tab === 'Dashboard' || tab === 'Order Service' || tab === 'Diagnostics' || tab === 'Assistant') {
      setInspectionState(null);
      setCompletedReport(null);
      setView(tab as View);
