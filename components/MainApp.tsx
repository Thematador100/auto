...
type View = 'Dashboard' | 'Inspection' | 'Diagnostics' | 'Assistant' | 'Finalize' | 'Report' | 'Admin' | 'Pricing' | 'Settings';
...
export const MainApp: React.FC<MainAppProps> = ({ user, onLogout }) => {
  ...
  const handleTabChange = (tab: View | string) => {
    ...
    if (tab === 'Dashboard' || tab === 'Diagnostics' || tab === 'Assistant' || tab === 'Admin' || tab === 'Pricing' || tab === 'Settings') {
      setInspectionState(null);
      setCompletedReport(null);
      setView(tab as View);
